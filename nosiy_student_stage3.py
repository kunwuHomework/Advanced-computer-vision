...
trainset100 = torchvision.datasets.CIFAR100(root='./data', train=True,
                                        download=True, transform=transform)

trainset1002 = torchvision.datasets.CIFAR100(root='./data', train=True,
                                        download=True, transform=transforms.ToTensor())
# Data loader
train_loader = torch.utils.data.DataLoader(dataset=trainset,
                                           batch_size=50, 
                                           shuffle=True)
train_loader2 = torch.utils.data.DataLoader(dataset=trainset100,
                                           batch_size=50, 
                                           shuffle=True)
train_loader3 = torch.utils.data.DataLoader(dataset=trainset1002,
                                           batch_size=50, 
                                           shuffle=True)
...
model_name = 'ns_nn_vgg19_sp_model'
...
for epoch in range(num_epochs):
    ...
    for i, (images, labels) in enumerate(train_loader2):
        ...
        #print(labels)
        _ , new_labels = torch.max(output1.data, 1)
        loss2 = criterion(outputs, new_labels)
    ...
    with torch.no_grad():
        ...
        if best_accuracy2> correct2 / total2:
            ...
            net_opt2 = model2
            torch.save(net_opt2,'ns_nn_vgg19_sp_model_v2')
